...
Graphe nouveau_graphe(int nb_sommets, Booleen est_oriente){
    ...
    int i ; // compteur pour initialiser les listes d'adjacence
    ...
    element = malloc (sizeof(*element ));
     ...
     if (element -> est_oriente){
        element -> graph_file = fopen ("digraph.dot","w"); // GO
     }else{
        element -> graph_file = fopen ("graph.dot","w") ;// GNO
     }
